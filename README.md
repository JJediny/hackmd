...
1. Install [node version manager (nvm)](https://github.com/nvm-sh/nvm) to control local development environment's NodeJS version or somehow use `Latest LTS Version: 10.16.0` codename `lts/dubnium` which is the long-term-supported (lts) version of NodeJS version 10. Upstream project is starting to work on support for LTS for v 12.x.x but the application is not yet compatiable. 
